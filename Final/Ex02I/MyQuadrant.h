...
class MyQuadrant
{
	static uint m_uQuadrantCount; //will store the number of octants instantiated
	...
	MyQuadrant* m_pParent = nullptr;// Will store the parent of current octant
	MyQuadrant* m_pChild[4];//Will store the children of the current octant

	std::vector<uint> m_EntityList; //List of Entities under this octant (Index in Entity Manager)

	MyQuadrant* m_pRoot = nullptr;//Root octant
	std::vector<MyQuadrant*> m_lChild; //list of nodes that contain objects (this will be applied to root only)
...
public:
	...
	OUTPUT: class object
	*/
	MyQuadrant(uint a_nMaxLevel = 2, uint a_nIdealEntityCount = 5, bool a_bIgnoreARBB = false);
	/*
	USAGE: Constructor
	ARGUMENTS:
	...
	OUTPUT: class object
	*/
	MyQuadrant(vector3 a_v3Center, float a_fSize);
	/*
	USAGE: Copy Constructor
	ARGUMENTS: class object to copy
	OUTPUT: class object instance
	*/
	MyQuadrant(MyQuadrant const& other);
	/*
	USAGE: Copy Assignment Operator
	ARGUMENTS: class object to copy
	OUTPUT: ---
	*/
	MyQuadrant& operator=(MyQuadrant const& other);
	/*
	USAGE: Destructor
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	...
	OUTPUT: ---
	*/
	void Swap(MyQuadrant& other);
	/*
	USAGE: Calculate the quatrant's size
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void Resize(void);
	/*
	USAGE: Gets this quatrant's size
	ARGUMENTS: ---
