...
#pragma region Your Code Goes Here
	const int defMax = 100000;
	m_v3MaxG = vector3(-defMax);
	m_v3MinG = vector3(defMax);

	vector3 vectors[8];
	vectors[0] = m_v3MinL;
	vectors[1] = m_v3MaxL;
	vectors[2] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MinL.z);
	vectors[3] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MinL.z);
	vectors[4] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MinL.z);
	vectors[5] = vector3(m_v3MinL.x, m_v3MinL.y, m_v3MaxL.z);
	vectors[6] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MaxL.z);
	vectors[7] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MaxL.z);

	for (size_t i = 0; i < 8; i++)
	{
		vectors[i] = m_m4ToWorld * vector4(vectors[i], 1);
		//x
		if (m_v3MaxG.x < vectors[i].x) m_v3MaxG.x = vectors[i].x;
		else if (m_v3MinG.x > vectors[i].x) m_v3MinG.x = vectors[i].x;
		//y
		if (m_v3MaxG.y < vectors[i].y) m_v3MaxG.y = vectors[i].y;
		else if (m_v3MinG.y > vectors[i].y) m_v3MinG.y = vectors[i].y;
		//z
		if (m_v3MaxG.z < vectors[i].z) m_v3MaxG.z = vectors[i].z;
		else if (m_v3MinG.z > vectors[i].z) m_v3MinG.z = vectors[i].z;
